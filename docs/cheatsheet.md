...
|`docker attach <container name or container id>`| On attach notre console a celle du container, attention ctrl+c va kill le container |
|
...
|`packer init .`| Permet d'installer les deps et init le projet|
|`packer build .`| Permet de build l'image docker|
|`packer fmt .`| Formate le fichier packer|
|`packer validate .`| Valide la syntaxe du fichier packer|